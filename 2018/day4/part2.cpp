...
int main()
{
	...
	// map of guard's number to total minutes asleep
	std::map<unsigned, unsigned> total_minutes_asleep_map;
	// map of guard's number to array representing an hour containing counts
	// by minute of when they were asleep
	...
	int max_count = 0, max_count_minute = 60, guard_id = 0;
	for (const auto &pair : minute_by_minute_asleep_map) {
		for (unsigned i = 0; i < 60; i++) {
			if (minute_by_minute_asleep_map[pair.first][i] > max_count) {
				max_count = minute_by_minute_asleep_map[pair.first][i];
				max_count_minute = i;
				guard_id = pair.first;
			}
		}
	}
	std::cout << max_count_minute << '+' << guard_id << '\n';
	return 0;
}